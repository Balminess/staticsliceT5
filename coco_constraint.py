...
import re
import TSED
from utils import remove_line_numbers
...
class TSEDMonotonicConstraint(Constraint):
    def __init__(self, tokenizer, original_code, lang="java", eos_token_id=None):
        ...
        self.prev_score = 0.0
        self.last_checked_line_count = 0  # Track number of complete lines checked
    ...
    def update(self, token_id: int):
        ...
        # Count complete lines in generated text
        current_line_count = cleaned.count('\n')

        # Only check TSED when a new line is completed or at EOS
        if current_line_count > self.last_checked_line_count or token_id == self.eos_token_id:
            ...
            # Check monotonicity constraint at line boundary
            if score < self.prev_score:
                self.reset()
                return False, False, True  # stepped, completed, reset

            self.prev_score = score
            self.last_checked_line_count = current_line_count
    ...
    def reset(self):
        self.generated_tokens = []
        self.prev_score = 0.0
        self.last_checked_line_count = 0
        self._completed = False
    ...
    def copy(self, stateful=False):
        ...
        if stateful:
            ...
            copied.last_checked_line_count = self.last_checked_line_count
